
user/_mp1-part2-6:     file format elf64-littleriscv


Disassembly of section .text:

0000000000000000 <s2>:
#include "user/threads.h"

#define NULL 0

void s2(int signo)
{
   0:	7139                	addi	sp,sp,-64
   2:	fc06                	sd	ra,56(sp)
   4:	f822                	sd	s0,48(sp)
   6:	f426                	sd	s1,40(sp)
   8:	f04a                	sd	s2,32(sp)
   a:	ec4e                	sd	s3,24(sp)
   c:	e852                	sd	s4,16(sp)
   e:	e456                	sd	s5,8(sp)
  10:	0080                	addi	s0,sp,64
  12:	892a                	mv	s2,a0
    int i = 0;
  14:	4481                	li	s1,0
    while(1) {
        if(signo) printf(" (#) \n");
        else printf(" (@) \n");
  16:	00001a97          	auipc	s5,0x1
  1a:	f82a8a93          	addi	s5,s5,-126 # f98 <thread_kill+0x18>
        if(signo) printf(" (#) \n");
  1e:	00001a17          	auipc	s4,0x1
  22:	f72a0a13          	addi	s4,s4,-142 # f90 <thread_kill+0x10>
        i++;
        if (i == 3) {
  26:	498d                	li	s3,3
  28:	a00d                	j	4a <s2+0x4a>
        if(signo) printf(" (#) \n");
  2a:	8552                	mv	a0,s4
  2c:	00001097          	auipc	ra,0x1
  30:	97e080e7          	jalr	-1666(ra) # 9aa <printf>
        i++;
  34:	2485                	addiw	s1,s1,1
        if (i == 3) {
  36:	01349663          	bne	s1,s3,42 <s2+0x42>
            thread_exit();
  3a:	00001097          	auipc	ra,0x1
  3e:	c58080e7          	jalr	-936(ra) # c92 <thread_exit>
        }
        thread_yield();
  42:	00001097          	auipc	ra,0x1
  46:	e6a080e7          	jalr	-406(ra) # eac <thread_yield>
        if(signo) printf(" (#) \n");
  4a:	fe0910e3          	bnez	s2,2a <s2+0x2a>
        else printf(" (@) \n");
  4e:	8556                	mv	a0,s5
  50:	00001097          	auipc	ra,0x1
  54:	95a080e7          	jalr	-1702(ra) # 9aa <printf>
  58:	bff1                	j	34 <s2+0x34>

000000000000005a <s1>:
    }    
}

void s1(int signo)
{
  5a:	7179                	addi	sp,sp,-48
  5c:	f406                	sd	ra,40(sp)
  5e:	f022                	sd	s0,32(sp)
  60:	ec26                	sd	s1,24(sp)
  62:	e84a                	sd	s2,16(sp)
  64:	e44e                	sd	s3,8(sp)
  66:	e052                	sd	s4,0(sp)
  68:	1800                	addi	s0,sp,48
  6a:	892a                	mv	s2,a0
  6c:	448d                	li	s1,3
    int i = 0;
    while(1) {
        if(signo) printf(" (&) \n");
        else printf(" (*) \n");
  6e:	00001a17          	auipc	s4,0x1
  72:	f3aa0a13          	addi	s4,s4,-198 # fa8 <thread_kill+0x28>
        if(signo) printf(" (&) \n");
  76:	00001997          	auipc	s3,0x1
  7a:	f2a98993          	addi	s3,s3,-214 # fa0 <thread_kill+0x20>
  7e:	a821                	j	96 <s1+0x3c>
  80:	854e                	mv	a0,s3
  82:	00001097          	auipc	ra,0x1
  86:	928080e7          	jalr	-1752(ra) # 9aa <printf>
        i++;
        if (i == 3) {
  8a:	34fd                	addiw	s1,s1,-1
  8c:	cc89                	beqz	s1,a6 <s1+0x4c>
            return;
        }
        thread_yield();
  8e:	00001097          	auipc	ra,0x1
  92:	e1e080e7          	jalr	-482(ra) # eac <thread_yield>
        if(signo) printf(" (&) \n");
  96:	fe0915e3          	bnez	s2,80 <s1+0x26>
        else printf(" (*) \n");
  9a:	8552                	mv	a0,s4
  9c:	00001097          	auipc	ra,0x1
  a0:	90e080e7          	jalr	-1778(ra) # 9aa <printf>
  a4:	b7dd                	j	8a <s1+0x30>
    }    
}
  a6:	70a2                	ld	ra,40(sp)
  a8:	7402                	ld	s0,32(sp)
  aa:	64e2                	ld	s1,24(sp)
  ac:	6942                	ld	s2,16(sp)
  ae:	69a2                	ld	s3,8(sp)
  b0:	6a02                	ld	s4,0(sp)
  b2:	6145                	addi	sp,sp,48
  b4:	8082                	ret

00000000000000b6 <f5>:

void f5(void *arg)
{
  b6:	7179                	addi	sp,sp,-48
  b8:	f406                	sd	ra,40(sp)
  ba:	f022                	sd	s0,32(sp)
  bc:	ec26                	sd	s1,24(sp)
  be:	e84a                	sd	s2,16(sp)
  c0:	e44e                	sd	s3,8(sp)
  c2:	1800                	addi	s0,sp,48
    int i = 0;
  c4:	4481                	li	s1,0
    while (1) {
        printf(" ( ) \n");
  c6:	00001997          	auipc	s3,0x1
  ca:	eea98993          	addi	s3,s3,-278 # fb0 <thread_kill+0x30>
        if(++i == 1){
  ce:	4905                	li	s2,1
  d0:	a809                	j	e2 <f5+0x2c>
            thread_exit();
  d2:	00001097          	auipc	ra,0x1
  d6:	bc0080e7          	jalr	-1088(ra) # c92 <thread_exit>
        }
        thread_yield();
  da:	00001097          	auipc	ra,0x1
  de:	dd2080e7          	jalr	-558(ra) # eac <thread_yield>
        printf(" ( ) \n");
  e2:	854e                	mv	a0,s3
  e4:	00001097          	auipc	ra,0x1
  e8:	8c6080e7          	jalr	-1850(ra) # 9aa <printf>
        if(++i == 1){
  ec:	2485                	addiw	s1,s1,1
  ee:	ff2482e3          	beq	s1,s2,d2 <f5+0x1c>
  f2:	b7e5                	j	da <f5+0x24>

00000000000000f4 <f2>:
        thread_yield();
    }
}

void f2(void *arg)
{
  f4:	7179                	addi	sp,sp,-48
  f6:	f406                	sd	ra,40(sp)
  f8:	f022                	sd	s0,32(sp)
  fa:	ec26                	sd	s1,24(sp)
  fc:	e84a                	sd	s2,16(sp)
  fe:	e44e                	sd	s3,8(sp)
 100:	e052                	sd	s4,0(sp)
 102:	1800                	addi	s0,sp,48
    int i = 0;
 104:	4481                	li	s1,0
    while(1) {
        printf(" (o) \n");
 106:	00001a17          	auipc	s4,0x1
 10a:	eb2a0a13          	addi	s4,s4,-334 # fb8 <thread_kill+0x38>
        struct thread *t3 = thread_create(f3, NULL);
 10e:	00000997          	auipc	s3,0x0
 112:	15098993          	addi	s3,s3,336 # 25e <f3>
        thread_add_runqueue(t3);
        if (++i == 1) {
 116:	4905                	li	s2,1
 118:	a809                	j	12a <f2+0x36>
            thread_exit();
 11a:	00001097          	auipc	ra,0x1
 11e:	b78080e7          	jalr	-1160(ra) # c92 <thread_exit>
        }
        thread_yield();
 122:	00001097          	auipc	ra,0x1
 126:	d8a080e7          	jalr	-630(ra) # eac <thread_yield>
        printf(" (o) \n");
 12a:	8552                	mv	a0,s4
 12c:	00001097          	auipc	ra,0x1
 130:	87e080e7          	jalr	-1922(ra) # 9aa <printf>
        struct thread *t3 = thread_create(f3, NULL);
 134:	4581                	li	a1,0
 136:	854e                	mv	a0,s3
 138:	00001097          	auipc	ra,0x1
 13c:	a8a080e7          	jalr	-1398(ra) # bc2 <thread_create>
        thread_add_runqueue(t3);
 140:	00001097          	auipc	ra,0x1
 144:	af4080e7          	jalr	-1292(ra) # c34 <thread_add_runqueue>
        if (++i == 1) {
 148:	2485                	addiw	s1,s1,1
 14a:	fd2488e3          	beq	s1,s2,11a <f2+0x26>
 14e:	bfd1                	j	122 <f2+0x2e>

0000000000000150 <f1>:
    }
}

void f1(void *arg)
{
 150:	1141                	addi	sp,sp,-16
 152:	e406                	sd	ra,8(sp)
 154:	e022                	sd	s0,0(sp)
 156:	0800                	addi	s0,sp,16
    int i = 0;
    
    while(1) {
        printf("-( )-\n");
 158:	00001517          	auipc	a0,0x1
 15c:	e6850513          	addi	a0,a0,-408 # fc0 <thread_kill+0x40>
 160:	00001097          	auipc	ra,0x1
 164:	84a080e7          	jalr	-1974(ra) # 9aa <printf>
        struct thread *t2 = thread_create(f2, NULL);
 168:	4581                	li	a1,0
 16a:	00000517          	auipc	a0,0x0
 16e:	f8a50513          	addi	a0,a0,-118 # f4 <f2>
 172:	00001097          	auipc	ra,0x1
 176:	a50080e7          	jalr	-1456(ra) # bc2 <thread_create>
        thread_add_runqueue(t2);
 17a:	00001097          	auipc	ra,0x1
 17e:	aba080e7          	jalr	-1350(ra) # c34 <thread_add_runqueue>
        if (++i == 2) {
            return;
        }
        thread_yield();
 182:	00001097          	auipc	ra,0x1
 186:	d2a080e7          	jalr	-726(ra) # eac <thread_yield>
        printf("-( )-\n");
 18a:	00001517          	auipc	a0,0x1
 18e:	e3650513          	addi	a0,a0,-458 # fc0 <thread_kill+0x40>
 192:	00001097          	auipc	ra,0x1
 196:	818080e7          	jalr	-2024(ra) # 9aa <printf>
        struct thread *t2 = thread_create(f2, NULL);
 19a:	4581                	li	a1,0
 19c:	00000517          	auipc	a0,0x0
 1a0:	f5850513          	addi	a0,a0,-168 # f4 <f2>
 1a4:	00001097          	auipc	ra,0x1
 1a8:	a1e080e7          	jalr	-1506(ra) # bc2 <thread_create>
        thread_add_runqueue(t2);
 1ac:	00001097          	auipc	ra,0x1
 1b0:	a88080e7          	jalr	-1400(ra) # c34 <thread_add_runqueue>
    }
}
 1b4:	60a2                	ld	ra,8(sp)
 1b6:	6402                	ld	s0,0(sp)
 1b8:	0141                	addi	sp,sp,16
 1ba:	8082                	ret

00000000000001bc <f4>:
{
 1bc:	7139                	addi	sp,sp,-64
 1be:	fc06                	sd	ra,56(sp)
 1c0:	f822                	sd	s0,48(sp)
 1c2:	f426                	sd	s1,40(sp)
 1c4:	f04a                	sd	s2,32(sp)
 1c6:	ec4e                	sd	s3,24(sp)
 1c8:	e852                	sd	s4,16(sp)
 1ca:	e456                	sd	s5,8(sp)
 1cc:	0080                	addi	s0,sp,64
        printf(" (.) \n");
 1ce:	00001517          	auipc	a0,0x1
 1d2:	dfa50513          	addi	a0,a0,-518 # fc8 <thread_kill+0x48>
 1d6:	00000097          	auipc	ra,0x0
 1da:	7d4080e7          	jalr	2004(ra) # 9aa <printf>
        struct thread *t5 = thread_create(f5, NULL);
 1de:	4581                	li	a1,0
 1e0:	00000517          	auipc	a0,0x0
 1e4:	ed650513          	addi	a0,a0,-298 # b6 <f5>
 1e8:	00001097          	auipc	ra,0x1
 1ec:	9da080e7          	jalr	-1574(ra) # bc2 <thread_create>
 1f0:	84aa                	mv	s1,a0
        thread_add_runqueue(t5);
 1f2:	00001097          	auipc	ra,0x1
 1f6:	a42080e7          	jalr	-1470(ra) # c34 <thread_add_runqueue>
        thread_kill(t5, 1);
 1fa:	4585                	li	a1,1
 1fc:	8526                	mv	a0,s1
 1fe:	00001097          	auipc	ra,0x1
 202:	d82080e7          	jalr	-638(ra) # f80 <thread_kill>
        if(++i == 2){
 206:	4905                	li	s2,1
        printf(" (.) \n");
 208:	00001a97          	auipc	s5,0x1
 20c:	dc0a8a93          	addi	s5,s5,-576 # fc8 <thread_kill+0x48>
        struct thread *t5 = thread_create(f5, NULL);
 210:	00000a17          	auipc	s4,0x0
 214:	ea6a0a13          	addi	s4,s4,-346 # b6 <f5>
        if(++i == 2){
 218:	4989                	li	s3,2
        thread_yield();
 21a:	00001097          	auipc	ra,0x1
 21e:	c92080e7          	jalr	-878(ra) # eac <thread_yield>
        printf(" (.) \n");
 222:	8556                	mv	a0,s5
 224:	00000097          	auipc	ra,0x0
 228:	786080e7          	jalr	1926(ra) # 9aa <printf>
        struct thread *t5 = thread_create(f5, NULL);
 22c:	4581                	li	a1,0
 22e:	8552                	mv	a0,s4
 230:	00001097          	auipc	ra,0x1
 234:	992080e7          	jalr	-1646(ra) # bc2 <thread_create>
 238:	84aa                	mv	s1,a0
        thread_add_runqueue(t5);
 23a:	00001097          	auipc	ra,0x1
 23e:	9fa080e7          	jalr	-1542(ra) # c34 <thread_add_runqueue>
        thread_kill(t5, 1);
 242:	4585                	li	a1,1
 244:	8526                	mv	a0,s1
 246:	00001097          	auipc	ra,0x1
 24a:	d3a080e7          	jalr	-710(ra) # f80 <thread_kill>
        if(++i == 2){
 24e:	2905                	addiw	s2,s2,1
 250:	fd3915e3          	bne	s2,s3,21a <f4+0x5e>
            thread_exit();
 254:	00001097          	auipc	ra,0x1
 258:	a3e080e7          	jalr	-1474(ra) # c92 <thread_exit>
 25c:	bf7d                	j	21a <f4+0x5e>

000000000000025e <f3>:
{
 25e:	711d                	addi	sp,sp,-96
 260:	ec86                	sd	ra,88(sp)
 262:	e8a2                	sd	s0,80(sp)
 264:	e4a6                	sd	s1,72(sp)
 266:	e0ca                	sd	s2,64(sp)
 268:	fc4e                	sd	s3,56(sp)
 26a:	f852                	sd	s4,48(sp)
 26c:	f456                	sd	s5,40(sp)
 26e:	f05a                	sd	s6,32(sp)
 270:	ec5e                	sd	s7,24(sp)
 272:	e862                	sd	s8,16(sp)
 274:	e466                	sd	s9,8(sp)
 276:	1080                	addi	s0,sp,96
 278:	4905                	li	s2,1
 27a:	4481                	li	s1,0
        printf(" (-) \n");
 27c:	00001b17          	auipc	s6,0x1
 280:	d54b0b13          	addi	s6,s6,-684 # fd0 <thread_kill+0x50>
        else if(i == 1){
 284:	4a85                	li	s5,1
            thread_register_handler(0, s2);
 286:	00000c17          	auipc	s8,0x0
 28a:	d7ac0c13          	addi	s8,s8,-646 # 0 <s2>
            struct thread *t4 = thread_create(f4, NULL);
 28e:	00000a17          	auipc	s4,0x0
 292:	f2ea0a13          	addi	s4,s4,-210 # 1bc <f4>
        if(++i == 3){
 296:	4b8d                	li	s7,3
            thread_register_handler(0, s1);
 298:	00000c97          	auipc	s9,0x0
 29c:	dc2c8c93          	addi	s9,s9,-574 # 5a <s1>
 2a0:	a0b5                	j	30c <f3+0xae>
            struct thread *t4 = thread_create(f4, NULL);
 2a2:	4581                	li	a1,0
 2a4:	8552                	mv	a0,s4
 2a6:	00001097          	auipc	ra,0x1
 2aa:	91c080e7          	jalr	-1764(ra) # bc2 <thread_create>
 2ae:	89aa                	mv	s3,a0
            thread_add_runqueue(t4);
 2b0:	00001097          	auipc	ra,0x1
 2b4:	984080e7          	jalr	-1660(ra) # c34 <thread_add_runqueue>
            thread_kill(t4, 1);
 2b8:	85d6                	mv	a1,s5
 2ba:	854e                	mv	a0,s3
 2bc:	00001097          	auipc	ra,0x1
 2c0:	cc4080e7          	jalr	-828(ra) # f80 <thread_kill>
 2c4:	a835                	j	300 <f3+0xa2>
            thread_register_handler(0, s2);
 2c6:	85e2                	mv	a1,s8
 2c8:	4501                	li	a0,0
 2ca:	00001097          	auipc	ra,0x1
 2ce:	c8c080e7          	jalr	-884(ra) # f56 <thread_register_handler>
            struct thread *t4 = thread_create(f4, NULL);
 2d2:	4581                	li	a1,0
 2d4:	8552                	mv	a0,s4
 2d6:	00001097          	auipc	ra,0x1
 2da:	8ec080e7          	jalr	-1812(ra) # bc2 <thread_create>
 2de:	89aa                	mv	s3,a0
            thread_add_runqueue(t4);
 2e0:	00001097          	auipc	ra,0x1
 2e4:	954080e7          	jalr	-1708(ra) # c34 <thread_add_runqueue>
            thread_kill(t4, 0);
 2e8:	4581                	li	a1,0
 2ea:	854e                	mv	a0,s3
 2ec:	00001097          	auipc	ra,0x1
 2f0:	c94080e7          	jalr	-876(ra) # f80 <thread_kill>
        if(++i == 3){
 2f4:	01791663          	bne	s2,s7,300 <f3+0xa2>
            thread_exit();
 2f8:	00001097          	auipc	ra,0x1
 2fc:	99a080e7          	jalr	-1638(ra) # c92 <thread_exit>
        thread_yield();
 300:	00001097          	auipc	ra,0x1
 304:	bac080e7          	jalr	-1108(ra) # eac <thread_yield>
    while (1) {
 308:	2485                	addiw	s1,s1,1
 30a:	2905                	addiw	s2,s2,1
        printf(" (-) \n");
 30c:	855a                	mv	a0,s6
 30e:	00000097          	auipc	ra,0x0
 312:	69c080e7          	jalr	1692(ra) # 9aa <printf>
        if(i == 0){
 316:	0004879b          	sext.w	a5,s1
 31a:	d7c1                	beqz	a5,2a2 <f3+0x44>
        else if(i == 1){
 31c:	fb5795e3          	bne	a5,s5,2c6 <f3+0x68>
            thread_register_handler(0, s1);
 320:	85e6                	mv	a1,s9
 322:	4501                	li	a0,0
 324:	00001097          	auipc	ra,0x1
 328:	c32080e7          	jalr	-974(ra) # f56 <thread_register_handler>
            struct thread *t4 = thread_create(f4, NULL);
 32c:	4581                	li	a1,0
 32e:	8552                	mv	a0,s4
 330:	00001097          	auipc	ra,0x1
 334:	892080e7          	jalr	-1902(ra) # bc2 <thread_create>
 338:	89aa                	mv	s3,a0
            thread_add_runqueue(t4);
 33a:	00001097          	auipc	ra,0x1
 33e:	8fa080e7          	jalr	-1798(ra) # c34 <thread_add_runqueue>
            thread_kill(t4, 0);
 342:	4581                	li	a1,0
 344:	854e                	mv	a0,s3
 346:	00001097          	auipc	ra,0x1
 34a:	c3a080e7          	jalr	-966(ra) # f80 <thread_kill>
 34e:	bf4d                	j	300 <f3+0xa2>

0000000000000350 <main>:

int main(int argc, char **argv)
{
 350:	1141                	addi	sp,sp,-16
 352:	e406                	sd	ra,8(sp)
 354:	e022                	sd	s0,0(sp)
 356:	0800                	addi	s0,sp,16
    printf("mp1-part2-6\n");
 358:	00001517          	auipc	a0,0x1
 35c:	c8050513          	addi	a0,a0,-896 # fd8 <thread_kill+0x58>
 360:	00000097          	auipc	ra,0x0
 364:	64a080e7          	jalr	1610(ra) # 9aa <printf>
    printf(" \\ / \n");
 368:	00001517          	auipc	a0,0x1
 36c:	c8050513          	addi	a0,a0,-896 # fe8 <thread_kill+0x68>
 370:	00000097          	auipc	ra,0x0
 374:	63a080e7          	jalr	1594(ra) # 9aa <printf>
    struct thread *t1 = thread_create(f1, NULL);
 378:	4581                	li	a1,0
 37a:	00000517          	auipc	a0,0x0
 37e:	dd650513          	addi	a0,a0,-554 # 150 <f1>
 382:	00001097          	auipc	ra,0x1
 386:	840080e7          	jalr	-1984(ra) # bc2 <thread_create>
    thread_add_runqueue(t1);
 38a:	00001097          	auipc	ra,0x1
 38e:	8aa080e7          	jalr	-1878(ra) # c34 <thread_add_runqueue>
    thread_start_threading();
 392:	00001097          	auipc	ra,0x1
 396:	b98080e7          	jalr	-1128(ra) # f2a <thread_start_threading>
    printf(" / \\ \n");
 39a:	00001517          	auipc	a0,0x1
 39e:	c5650513          	addi	a0,a0,-938 # ff0 <thread_kill+0x70>
 3a2:	00000097          	auipc	ra,0x0
 3a6:	608080e7          	jalr	1544(ra) # 9aa <printf>
    printf("\nexited\n");
 3aa:	00001517          	auipc	a0,0x1
 3ae:	c4e50513          	addi	a0,a0,-946 # ff8 <thread_kill+0x78>
 3b2:	00000097          	auipc	ra,0x0
 3b6:	5f8080e7          	jalr	1528(ra) # 9aa <printf>
    exit(0);
 3ba:	4501                	li	a0,0
 3bc:	00000097          	auipc	ra,0x0
 3c0:	276080e7          	jalr	630(ra) # 632 <exit>

00000000000003c4 <strcpy>:
#include "kernel/fcntl.h"
#include "user/user.h"

char*
strcpy(char *s, const char *t)
{
 3c4:	1141                	addi	sp,sp,-16
 3c6:	e422                	sd	s0,8(sp)
 3c8:	0800                	addi	s0,sp,16
  char *os;

  os = s;
  while((*s++ = *t++) != 0)
 3ca:	87aa                	mv	a5,a0
 3cc:	0585                	addi	a1,a1,1
 3ce:	0785                	addi	a5,a5,1
 3d0:	fff5c703          	lbu	a4,-1(a1)
 3d4:	fee78fa3          	sb	a4,-1(a5)
 3d8:	fb75                	bnez	a4,3cc <strcpy+0x8>
    ;
  return os;
}
 3da:	6422                	ld	s0,8(sp)
 3dc:	0141                	addi	sp,sp,16
 3de:	8082                	ret

00000000000003e0 <strcmp>:

int
strcmp(const char *p, const char *q)
{
 3e0:	1141                	addi	sp,sp,-16
 3e2:	e422                	sd	s0,8(sp)
 3e4:	0800                	addi	s0,sp,16
  while(*p && *p == *q)
 3e6:	00054783          	lbu	a5,0(a0)
 3ea:	cb91                	beqz	a5,3fe <strcmp+0x1e>
 3ec:	0005c703          	lbu	a4,0(a1)
 3f0:	00f71763          	bne	a4,a5,3fe <strcmp+0x1e>
    p++, q++;
 3f4:	0505                	addi	a0,a0,1
 3f6:	0585                	addi	a1,a1,1
  while(*p && *p == *q)
 3f8:	00054783          	lbu	a5,0(a0)
 3fc:	fbe5                	bnez	a5,3ec <strcmp+0xc>
  return (uchar)*p - (uchar)*q;
 3fe:	0005c503          	lbu	a0,0(a1)
}
 402:	40a7853b          	subw	a0,a5,a0
 406:	6422                	ld	s0,8(sp)
 408:	0141                	addi	sp,sp,16
 40a:	8082                	ret

000000000000040c <strlen>:

uint
strlen(const char *s)
{
 40c:	1141                	addi	sp,sp,-16
 40e:	e422                	sd	s0,8(sp)
 410:	0800                	addi	s0,sp,16
  int n;

  for(n = 0; s[n]; n++)
 412:	00054783          	lbu	a5,0(a0)
 416:	cf91                	beqz	a5,432 <strlen+0x26>
 418:	0505                	addi	a0,a0,1
 41a:	87aa                	mv	a5,a0
 41c:	4685                	li	a3,1
 41e:	9e89                	subw	a3,a3,a0
 420:	00f6853b          	addw	a0,a3,a5
 424:	0785                	addi	a5,a5,1
 426:	fff7c703          	lbu	a4,-1(a5)
 42a:	fb7d                	bnez	a4,420 <strlen+0x14>
    ;
  return n;
}
 42c:	6422                	ld	s0,8(sp)
 42e:	0141                	addi	sp,sp,16
 430:	8082                	ret
  for(n = 0; s[n]; n++)
 432:	4501                	li	a0,0
 434:	bfe5                	j	42c <strlen+0x20>

0000000000000436 <memset>:

void*
memset(void *dst, int c, uint n)
{
 436:	1141                	addi	sp,sp,-16
 438:	e422                	sd	s0,8(sp)
 43a:	0800                	addi	s0,sp,16
  char *cdst = (char *) dst;
  int i;
  for(i = 0; i < n; i++){
 43c:	ca19                	beqz	a2,452 <memset+0x1c>
 43e:	87aa                	mv	a5,a0
 440:	1602                	slli	a2,a2,0x20
 442:	9201                	srli	a2,a2,0x20
 444:	00a60733          	add	a4,a2,a0
    cdst[i] = c;
 448:	00b78023          	sb	a1,0(a5)
  for(i = 0; i < n; i++){
 44c:	0785                	addi	a5,a5,1
 44e:	fee79de3          	bne	a5,a4,448 <memset+0x12>
  }
  return dst;
}
 452:	6422                	ld	s0,8(sp)
 454:	0141                	addi	sp,sp,16
 456:	8082                	ret

0000000000000458 <strchr>:

char*
strchr(const char *s, char c)
{
 458:	1141                	addi	sp,sp,-16
 45a:	e422                	sd	s0,8(sp)
 45c:	0800                	addi	s0,sp,16
  for(; *s; s++)
 45e:	00054783          	lbu	a5,0(a0)
 462:	cb99                	beqz	a5,478 <strchr+0x20>
    if(*s == c)
 464:	00f58763          	beq	a1,a5,472 <strchr+0x1a>
  for(; *s; s++)
 468:	0505                	addi	a0,a0,1
 46a:	00054783          	lbu	a5,0(a0)
 46e:	fbfd                	bnez	a5,464 <strchr+0xc>
      return (char*)s;
  return 0;
 470:	4501                	li	a0,0
}
 472:	6422                	ld	s0,8(sp)
 474:	0141                	addi	sp,sp,16
 476:	8082                	ret
  return 0;
 478:	4501                	li	a0,0
 47a:	bfe5                	j	472 <strchr+0x1a>

000000000000047c <gets>:

char*
gets(char *buf, int max)
{
 47c:	711d                	addi	sp,sp,-96
 47e:	ec86                	sd	ra,88(sp)
 480:	e8a2                	sd	s0,80(sp)
 482:	e4a6                	sd	s1,72(sp)
 484:	e0ca                	sd	s2,64(sp)
 486:	fc4e                	sd	s3,56(sp)
 488:	f852                	sd	s4,48(sp)
 48a:	f456                	sd	s5,40(sp)
 48c:	f05a                	sd	s6,32(sp)
 48e:	ec5e                	sd	s7,24(sp)
 490:	1080                	addi	s0,sp,96
 492:	8baa                	mv	s7,a0
 494:	8a2e                	mv	s4,a1
  int i, cc;
  char c;

  for(i=0; i+1 < max; ){
 496:	892a                	mv	s2,a0
 498:	4481                	li	s1,0
    cc = read(0, &c, 1);
    if(cc < 1)
      break;
    buf[i++] = c;
    if(c == '\n' || c == '\r')
 49a:	4aa9                	li	s5,10
 49c:	4b35                	li	s6,13
  for(i=0; i+1 < max; ){
 49e:	89a6                	mv	s3,s1
 4a0:	2485                	addiw	s1,s1,1
 4a2:	0344d863          	bge	s1,s4,4d2 <gets+0x56>
    cc = read(0, &c, 1);
 4a6:	4605                	li	a2,1
 4a8:	faf40593          	addi	a1,s0,-81
 4ac:	4501                	li	a0,0
 4ae:	00000097          	auipc	ra,0x0
 4b2:	19c080e7          	jalr	412(ra) # 64a <read>
    if(cc < 1)
 4b6:	00a05e63          	blez	a0,4d2 <gets+0x56>
    buf[i++] = c;
 4ba:	faf44783          	lbu	a5,-81(s0)
 4be:	00f90023          	sb	a5,0(s2)
    if(c == '\n' || c == '\r')
 4c2:	01578763          	beq	a5,s5,4d0 <gets+0x54>
 4c6:	0905                	addi	s2,s2,1
 4c8:	fd679be3          	bne	a5,s6,49e <gets+0x22>
  for(i=0; i+1 < max; ){
 4cc:	89a6                	mv	s3,s1
 4ce:	a011                	j	4d2 <gets+0x56>
 4d0:	89a6                	mv	s3,s1
      break;
  }
  buf[i] = '\0';
 4d2:	99de                	add	s3,s3,s7
 4d4:	00098023          	sb	zero,0(s3)
  return buf;
}
 4d8:	855e                	mv	a0,s7
 4da:	60e6                	ld	ra,88(sp)
 4dc:	6446                	ld	s0,80(sp)
 4de:	64a6                	ld	s1,72(sp)
 4e0:	6906                	ld	s2,64(sp)
 4e2:	79e2                	ld	s3,56(sp)
 4e4:	7a42                	ld	s4,48(sp)
 4e6:	7aa2                	ld	s5,40(sp)
 4e8:	7b02                	ld	s6,32(sp)
 4ea:	6be2                	ld	s7,24(sp)
 4ec:	6125                	addi	sp,sp,96
 4ee:	8082                	ret

00000000000004f0 <stat>:

int
stat(const char *n, struct stat *st)
{
 4f0:	1101                	addi	sp,sp,-32
 4f2:	ec06                	sd	ra,24(sp)
 4f4:	e822                	sd	s0,16(sp)
 4f6:	e426                	sd	s1,8(sp)
 4f8:	e04a                	sd	s2,0(sp)
 4fa:	1000                	addi	s0,sp,32
 4fc:	892e                	mv	s2,a1
  int fd;
  int r;

  fd = open(n, O_RDONLY);
 4fe:	4581                	li	a1,0
 500:	00000097          	auipc	ra,0x0
 504:	172080e7          	jalr	370(ra) # 672 <open>
  if(fd < 0)
 508:	02054563          	bltz	a0,532 <stat+0x42>
 50c:	84aa                	mv	s1,a0
    return -1;
  r = fstat(fd, st);
 50e:	85ca                	mv	a1,s2
 510:	00000097          	auipc	ra,0x0
 514:	17a080e7          	jalr	378(ra) # 68a <fstat>
 518:	892a                	mv	s2,a0
  close(fd);
 51a:	8526                	mv	a0,s1
 51c:	00000097          	auipc	ra,0x0
 520:	13e080e7          	jalr	318(ra) # 65a <close>
  return r;
}
 524:	854a                	mv	a0,s2
 526:	60e2                	ld	ra,24(sp)
 528:	6442                	ld	s0,16(sp)
 52a:	64a2                	ld	s1,8(sp)
 52c:	6902                	ld	s2,0(sp)
 52e:	6105                	addi	sp,sp,32
 530:	8082                	ret
    return -1;
 532:	597d                	li	s2,-1
 534:	bfc5                	j	524 <stat+0x34>

0000000000000536 <atoi>:

int
atoi(const char *s)
{
 536:	1141                	addi	sp,sp,-16
 538:	e422                	sd	s0,8(sp)
 53a:	0800                	addi	s0,sp,16
  int n;

  n = 0;
  while('0' <= *s && *s <= '9')
 53c:	00054603          	lbu	a2,0(a0)
 540:	fd06079b          	addiw	a5,a2,-48
 544:	0ff7f793          	andi	a5,a5,255
 548:	4725                	li	a4,9
 54a:	02f76963          	bltu	a4,a5,57c <atoi+0x46>
 54e:	86aa                	mv	a3,a0
  n = 0;
 550:	4501                	li	a0,0
  while('0' <= *s && *s <= '9')
 552:	45a5                	li	a1,9
    n = n*10 + *s++ - '0';
 554:	0685                	addi	a3,a3,1
 556:	0025179b          	slliw	a5,a0,0x2
 55a:	9fa9                	addw	a5,a5,a0
 55c:	0017979b          	slliw	a5,a5,0x1
 560:	9fb1                	addw	a5,a5,a2
 562:	fd07851b          	addiw	a0,a5,-48
  while('0' <= *s && *s <= '9')
 566:	0006c603          	lbu	a2,0(a3)
 56a:	fd06071b          	addiw	a4,a2,-48
 56e:	0ff77713          	andi	a4,a4,255
 572:	fee5f1e3          	bgeu	a1,a4,554 <atoi+0x1e>
  return n;
}
 576:	6422                	ld	s0,8(sp)
 578:	0141                	addi	sp,sp,16
 57a:	8082                	ret
  n = 0;
 57c:	4501                	li	a0,0
 57e:	bfe5                	j	576 <atoi+0x40>

0000000000000580 <memmove>:

void*
memmove(void *vdst, const void *vsrc, int n)
{
 580:	1141                	addi	sp,sp,-16
 582:	e422                	sd	s0,8(sp)
 584:	0800                	addi	s0,sp,16
  char *dst;
  const char *src;

  dst = vdst;
  src = vsrc;
  if (src > dst) {
 586:	02b57463          	bgeu	a0,a1,5ae <memmove+0x2e>
    while(n-- > 0)
 58a:	00c05f63          	blez	a2,5a8 <memmove+0x28>
 58e:	1602                	slli	a2,a2,0x20
 590:	9201                	srli	a2,a2,0x20
 592:	00c507b3          	add	a5,a0,a2
  dst = vdst;
 596:	872a                	mv	a4,a0
      *dst++ = *src++;
 598:	0585                	addi	a1,a1,1
 59a:	0705                	addi	a4,a4,1
 59c:	fff5c683          	lbu	a3,-1(a1)
 5a0:	fed70fa3          	sb	a3,-1(a4)
    while(n-- > 0)
 5a4:	fee79ae3          	bne	a5,a4,598 <memmove+0x18>
    src += n;
    while(n-- > 0)
      *--dst = *--src;
  }
  return vdst;
}
 5a8:	6422                	ld	s0,8(sp)
 5aa:	0141                	addi	sp,sp,16
 5ac:	8082                	ret
    dst += n;
 5ae:	00c50733          	add	a4,a0,a2
    src += n;
 5b2:	95b2                	add	a1,a1,a2
    while(n-- > 0)
 5b4:	fec05ae3          	blez	a2,5a8 <memmove+0x28>
 5b8:	fff6079b          	addiw	a5,a2,-1
 5bc:	1782                	slli	a5,a5,0x20
 5be:	9381                	srli	a5,a5,0x20
 5c0:	fff7c793          	not	a5,a5
 5c4:	97ba                	add	a5,a5,a4
      *--dst = *--src;
 5c6:	15fd                	addi	a1,a1,-1
 5c8:	177d                	addi	a4,a4,-1
 5ca:	0005c683          	lbu	a3,0(a1)
 5ce:	00d70023          	sb	a3,0(a4)
    while(n-- > 0)
 5d2:	fee79ae3          	bne	a5,a4,5c6 <memmove+0x46>
 5d6:	bfc9                	j	5a8 <memmove+0x28>

00000000000005d8 <memcmp>:

int
memcmp(const void *s1, const void *s2, uint n)
{
 5d8:	1141                	addi	sp,sp,-16
 5da:	e422                	sd	s0,8(sp)
 5dc:	0800                	addi	s0,sp,16
  const char *p1 = s1, *p2 = s2;
  while (n-- > 0) {
 5de:	ca05                	beqz	a2,60e <memcmp+0x36>
 5e0:	fff6069b          	addiw	a3,a2,-1
 5e4:	1682                	slli	a3,a3,0x20
 5e6:	9281                	srli	a3,a3,0x20
 5e8:	0685                	addi	a3,a3,1
 5ea:	96aa                	add	a3,a3,a0
    if (*p1 != *p2) {
 5ec:	00054783          	lbu	a5,0(a0)
 5f0:	0005c703          	lbu	a4,0(a1)
 5f4:	00e79863          	bne	a5,a4,604 <memcmp+0x2c>
      return *p1 - *p2;
    }
    p1++;
 5f8:	0505                	addi	a0,a0,1
    p2++;
 5fa:	0585                	addi	a1,a1,1
  while (n-- > 0) {
 5fc:	fed518e3          	bne	a0,a3,5ec <memcmp+0x14>
  }
  return 0;
 600:	4501                	li	a0,0
 602:	a019                	j	608 <memcmp+0x30>
      return *p1 - *p2;
 604:	40e7853b          	subw	a0,a5,a4
}
 608:	6422                	ld	s0,8(sp)
 60a:	0141                	addi	sp,sp,16
 60c:	8082                	ret
  return 0;
 60e:	4501                	li	a0,0
 610:	bfe5                	j	608 <memcmp+0x30>

0000000000000612 <memcpy>:

void *
memcpy(void *dst, const void *src, uint n)
{
 612:	1141                	addi	sp,sp,-16
 614:	e406                	sd	ra,8(sp)
 616:	e022                	sd	s0,0(sp)
 618:	0800                	addi	s0,sp,16
  return memmove(dst, src, n);
 61a:	00000097          	auipc	ra,0x0
 61e:	f66080e7          	jalr	-154(ra) # 580 <memmove>
}
 622:	60a2                	ld	ra,8(sp)
 624:	6402                	ld	s0,0(sp)
 626:	0141                	addi	sp,sp,16
 628:	8082                	ret

000000000000062a <fork>:
# generated by usys.pl - do not edit
#include "kernel/syscall.h"
.global fork
fork:
 li a7, SYS_fork
 62a:	4885                	li	a7,1
 ecall
 62c:	00000073          	ecall
 ret
 630:	8082                	ret

0000000000000632 <exit>:
.global exit
exit:
 li a7, SYS_exit
 632:	4889                	li	a7,2
 ecall
 634:	00000073          	ecall
 ret
 638:	8082                	ret

000000000000063a <wait>:
.global wait
wait:
 li a7, SYS_wait
 63a:	488d                	li	a7,3
 ecall
 63c:	00000073          	ecall
 ret
 640:	8082                	ret

0000000000000642 <pipe>:
.global pipe
pipe:
 li a7, SYS_pipe
 642:	4891                	li	a7,4
 ecall
 644:	00000073          	ecall
 ret
 648:	8082                	ret

000000000000064a <read>:
.global read
read:
 li a7, SYS_read
 64a:	4895                	li	a7,5
 ecall
 64c:	00000073          	ecall
 ret
 650:	8082                	ret

0000000000000652 <write>:
.global write
write:
 li a7, SYS_write
 652:	48c1                	li	a7,16
 ecall
 654:	00000073          	ecall
 ret
 658:	8082                	ret

000000000000065a <close>:
.global close
close:
 li a7, SYS_close
 65a:	48d5                	li	a7,21
 ecall
 65c:	00000073          	ecall
 ret
 660:	8082                	ret

0000000000000662 <kill>:
.global kill
kill:
 li a7, SYS_kill
 662:	4899                	li	a7,6
 ecall
 664:	00000073          	ecall
 ret
 668:	8082                	ret

000000000000066a <exec>:
.global exec
exec:
 li a7, SYS_exec
 66a:	489d                	li	a7,7
 ecall
 66c:	00000073          	ecall
 ret
 670:	8082                	ret

0000000000000672 <open>:
.global open
open:
 li a7, SYS_open
 672:	48bd                	li	a7,15
 ecall
 674:	00000073          	ecall
 ret
 678:	8082                	ret

000000000000067a <mknod>:
.global mknod
mknod:
 li a7, SYS_mknod
 67a:	48c5                	li	a7,17
 ecall
 67c:	00000073          	ecall
 ret
 680:	8082                	ret

0000000000000682 <unlink>:
.global unlink
unlink:
 li a7, SYS_unlink
 682:	48c9                	li	a7,18
 ecall
 684:	00000073          	ecall
 ret
 688:	8082                	ret

000000000000068a <fstat>:
.global fstat
fstat:
 li a7, SYS_fstat
 68a:	48a1                	li	a7,8
 ecall
 68c:	00000073          	ecall
 ret
 690:	8082                	ret

0000000000000692 <link>:
.global link
link:
 li a7, SYS_link
 692:	48cd                	li	a7,19
 ecall
 694:	00000073          	ecall
 ret
 698:	8082                	ret

000000000000069a <mkdir>:
.global mkdir
mkdir:
 li a7, SYS_mkdir
 69a:	48d1                	li	a7,20
 ecall
 69c:	00000073          	ecall
 ret
 6a0:	8082                	ret

00000000000006a2 <chdir>:
.global chdir
chdir:
 li a7, SYS_chdir
 6a2:	48a5                	li	a7,9
 ecall
 6a4:	00000073          	ecall
 ret
 6a8:	8082                	ret

00000000000006aa <dup>:
.global dup
dup:
 li a7, SYS_dup
 6aa:	48a9                	li	a7,10
 ecall
 6ac:	00000073          	ecall
 ret
 6b0:	8082                	ret

00000000000006b2 <getpid>:
.global getpid
getpid:
 li a7, SYS_getpid
 6b2:	48ad                	li	a7,11
 ecall
 6b4:	00000073          	ecall
 ret
 6b8:	8082                	ret

00000000000006ba <sbrk>:
.global sbrk
sbrk:
 li a7, SYS_sbrk
 6ba:	48b1                	li	a7,12
 ecall
 6bc:	00000073          	ecall
 ret
 6c0:	8082                	ret

00000000000006c2 <sleep>:
.global sleep
sleep:
 li a7, SYS_sleep
 6c2:	48b5                	li	a7,13
 ecall
 6c4:	00000073          	ecall
 ret
 6c8:	8082                	ret

00000000000006ca <uptime>:
.global uptime
uptime:
 li a7, SYS_uptime
 6ca:	48b9                	li	a7,14
 ecall
 6cc:	00000073          	ecall
 ret
 6d0:	8082                	ret

00000000000006d2 <putc>:

static char digits[] = "0123456789ABCDEF";

static void
putc(int fd, char c)
{
 6d2:	1101                	addi	sp,sp,-32
 6d4:	ec06                	sd	ra,24(sp)
 6d6:	e822                	sd	s0,16(sp)
 6d8:	1000                	addi	s0,sp,32
 6da:	feb407a3          	sb	a1,-17(s0)
  write(fd, &c, 1);
 6de:	4605                	li	a2,1
 6e0:	fef40593          	addi	a1,s0,-17
 6e4:	00000097          	auipc	ra,0x0
 6e8:	f6e080e7          	jalr	-146(ra) # 652 <write>
}
 6ec:	60e2                	ld	ra,24(sp)
 6ee:	6442                	ld	s0,16(sp)
 6f0:	6105                	addi	sp,sp,32
 6f2:	8082                	ret

00000000000006f4 <printint>:

static void
printint(int fd, int xx, int base, int sgn)
{
 6f4:	7139                	addi	sp,sp,-64
 6f6:	fc06                	sd	ra,56(sp)
 6f8:	f822                	sd	s0,48(sp)
 6fa:	f426                	sd	s1,40(sp)
 6fc:	f04a                	sd	s2,32(sp)
 6fe:	ec4e                	sd	s3,24(sp)
 700:	0080                	addi	s0,sp,64
 702:	84aa                	mv	s1,a0
  char buf[16];
  int i, neg;
  uint x;

  neg = 0;
  if(sgn && xx < 0){
 704:	c299                	beqz	a3,70a <printint+0x16>
 706:	0805c863          	bltz	a1,796 <printint+0xa2>
    neg = 1;
    x = -xx;
  } else {
    x = xx;
 70a:	2581                	sext.w	a1,a1
  neg = 0;
 70c:	4881                	li	a7,0
 70e:	fc040693          	addi	a3,s0,-64
  }

  i = 0;
 712:	4701                	li	a4,0
  do{
    buf[i++] = digits[x % base];
 714:	2601                	sext.w	a2,a2
 716:	00001517          	auipc	a0,0x1
 71a:	8fa50513          	addi	a0,a0,-1798 # 1010 <digits>
 71e:	883a                	mv	a6,a4
 720:	2705                	addiw	a4,a4,1
 722:	02c5f7bb          	remuw	a5,a1,a2
 726:	1782                	slli	a5,a5,0x20
 728:	9381                	srli	a5,a5,0x20
 72a:	97aa                	add	a5,a5,a0
 72c:	0007c783          	lbu	a5,0(a5)
 730:	00f68023          	sb	a5,0(a3)
  }while((x /= base) != 0);
 734:	0005879b          	sext.w	a5,a1
 738:	02c5d5bb          	divuw	a1,a1,a2
 73c:	0685                	addi	a3,a3,1
 73e:	fec7f0e3          	bgeu	a5,a2,71e <printint+0x2a>
  if(neg)
 742:	00088b63          	beqz	a7,758 <printint+0x64>
    buf[i++] = '-';
 746:	fd040793          	addi	a5,s0,-48
 74a:	973e                	add	a4,a4,a5
 74c:	02d00793          	li	a5,45
 750:	fef70823          	sb	a5,-16(a4)
 754:	0028071b          	addiw	a4,a6,2

  while(--i >= 0)
 758:	02e05863          	blez	a4,788 <printint+0x94>
 75c:	fc040793          	addi	a5,s0,-64
 760:	00e78933          	add	s2,a5,a4
 764:	fff78993          	addi	s3,a5,-1
 768:	99ba                	add	s3,s3,a4
 76a:	377d                	addiw	a4,a4,-1
 76c:	1702                	slli	a4,a4,0x20
 76e:	9301                	srli	a4,a4,0x20
 770:	40e989b3          	sub	s3,s3,a4
    putc(fd, buf[i]);
 774:	fff94583          	lbu	a1,-1(s2)
 778:	8526                	mv	a0,s1
 77a:	00000097          	auipc	ra,0x0
 77e:	f58080e7          	jalr	-168(ra) # 6d2 <putc>
  while(--i >= 0)
 782:	197d                	addi	s2,s2,-1
 784:	ff3918e3          	bne	s2,s3,774 <printint+0x80>
}
 788:	70e2                	ld	ra,56(sp)
 78a:	7442                	ld	s0,48(sp)
 78c:	74a2                	ld	s1,40(sp)
 78e:	7902                	ld	s2,32(sp)
 790:	69e2                	ld	s3,24(sp)
 792:	6121                	addi	sp,sp,64
 794:	8082                	ret
    x = -xx;
 796:	40b005bb          	negw	a1,a1
    neg = 1;
 79a:	4885                	li	a7,1
    x = -xx;
 79c:	bf8d                	j	70e <printint+0x1a>

000000000000079e <vprintf>:
}

// Print to the given fd. Only understands %d, %x, %p, %s.
void
vprintf(int fd, const char *fmt, va_list ap)
{
 79e:	7119                	addi	sp,sp,-128
 7a0:	fc86                	sd	ra,120(sp)
 7a2:	f8a2                	sd	s0,112(sp)
 7a4:	f4a6                	sd	s1,104(sp)
 7a6:	f0ca                	sd	s2,96(sp)
 7a8:	ecce                	sd	s3,88(sp)
 7aa:	e8d2                	sd	s4,80(sp)
 7ac:	e4d6                	sd	s5,72(sp)
 7ae:	e0da                	sd	s6,64(sp)
 7b0:	fc5e                	sd	s7,56(sp)
 7b2:	f862                	sd	s8,48(sp)
 7b4:	f466                	sd	s9,40(sp)
 7b6:	f06a                	sd	s10,32(sp)
 7b8:	ec6e                	sd	s11,24(sp)
 7ba:	0100                	addi	s0,sp,128
  char *s;
  int c, i, state;

  state = 0;
  for(i = 0; fmt[i]; i++){
 7bc:	0005c903          	lbu	s2,0(a1)
 7c0:	18090f63          	beqz	s2,95e <vprintf+0x1c0>
 7c4:	8aaa                	mv	s5,a0
 7c6:	8b32                	mv	s6,a2
 7c8:	00158493          	addi	s1,a1,1
  state = 0;
 7cc:	4981                	li	s3,0
      if(c == '%'){
        state = '%';
      } else {
        putc(fd, c);
      }
    } else if(state == '%'){
 7ce:	02500a13          	li	s4,37
      if(c == 'd'){
 7d2:	06400c13          	li	s8,100
        printint(fd, va_arg(ap, int), 10, 1);
      } else if(c == 'l') {
 7d6:	06c00c93          	li	s9,108
        printint(fd, va_arg(ap, uint64), 10, 0);
      } else if(c == 'x') {
 7da:	07800d13          	li	s10,120
        printint(fd, va_arg(ap, int), 16, 0);
      } else if(c == 'p') {
 7de:	07000d93          	li	s11,112
    putc(fd, digits[x >> (sizeof(uint64) * 8 - 4)]);
 7e2:	00001b97          	auipc	s7,0x1
 7e6:	82eb8b93          	addi	s7,s7,-2002 # 1010 <digits>
 7ea:	a839                	j	808 <vprintf+0x6a>
        putc(fd, c);
 7ec:	85ca                	mv	a1,s2
 7ee:	8556                	mv	a0,s5
 7f0:	00000097          	auipc	ra,0x0
 7f4:	ee2080e7          	jalr	-286(ra) # 6d2 <putc>
 7f8:	a019                	j	7fe <vprintf+0x60>
    } else if(state == '%'){
 7fa:	01498f63          	beq	s3,s4,818 <vprintf+0x7a>
  for(i = 0; fmt[i]; i++){
 7fe:	0485                	addi	s1,s1,1
 800:	fff4c903          	lbu	s2,-1(s1)
 804:	14090d63          	beqz	s2,95e <vprintf+0x1c0>
    c = fmt[i] & 0xff;
 808:	0009079b          	sext.w	a5,s2
    if(state == 0){
 80c:	fe0997e3          	bnez	s3,7fa <vprintf+0x5c>
      if(c == '%'){
 810:	fd479ee3          	bne	a5,s4,7ec <vprintf+0x4e>
        state = '%';
 814:	89be                	mv	s3,a5
 816:	b7e5                	j	7fe <vprintf+0x60>
      if(c == 'd'){
 818:	05878063          	beq	a5,s8,858 <vprintf+0xba>
      } else if(c == 'l') {
 81c:	05978c63          	beq	a5,s9,874 <vprintf+0xd6>
      } else if(c == 'x') {
 820:	07a78863          	beq	a5,s10,890 <vprintf+0xf2>
      } else if(c == 'p') {
 824:	09b78463          	beq	a5,s11,8ac <vprintf+0x10e>
        printptr(fd, va_arg(ap, uint64));
      } else if(c == 's'){
 828:	07300713          	li	a4,115
 82c:	0ce78663          	beq	a5,a4,8f8 <vprintf+0x15a>
          s = "(null)";
        while(*s != 0){
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
 830:	06300713          	li	a4,99
 834:	0ee78e63          	beq	a5,a4,930 <vprintf+0x192>
        putc(fd, va_arg(ap, uint));
      } else if(c == '%'){
 838:	11478863          	beq	a5,s4,948 <vprintf+0x1aa>
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
        putc(fd, '%');
 83c:	85d2                	mv	a1,s4
 83e:	8556                	mv	a0,s5
 840:	00000097          	auipc	ra,0x0
 844:	e92080e7          	jalr	-366(ra) # 6d2 <putc>
        putc(fd, c);
 848:	85ca                	mv	a1,s2
 84a:	8556                	mv	a0,s5
 84c:	00000097          	auipc	ra,0x0
 850:	e86080e7          	jalr	-378(ra) # 6d2 <putc>
      }
      state = 0;
 854:	4981                	li	s3,0
 856:	b765                	j	7fe <vprintf+0x60>
        printint(fd, va_arg(ap, int), 10, 1);
 858:	008b0913          	addi	s2,s6,8
 85c:	4685                	li	a3,1
 85e:	4629                	li	a2,10
 860:	000b2583          	lw	a1,0(s6)
 864:	8556                	mv	a0,s5
 866:	00000097          	auipc	ra,0x0
 86a:	e8e080e7          	jalr	-370(ra) # 6f4 <printint>
 86e:	8b4a                	mv	s6,s2
      state = 0;
 870:	4981                	li	s3,0
 872:	b771                	j	7fe <vprintf+0x60>
        printint(fd, va_arg(ap, uint64), 10, 0);
 874:	008b0913          	addi	s2,s6,8
 878:	4681                	li	a3,0
 87a:	4629                	li	a2,10
 87c:	000b2583          	lw	a1,0(s6)
 880:	8556                	mv	a0,s5
 882:	00000097          	auipc	ra,0x0
 886:	e72080e7          	jalr	-398(ra) # 6f4 <printint>
 88a:	8b4a                	mv	s6,s2
      state = 0;
 88c:	4981                	li	s3,0
 88e:	bf85                	j	7fe <vprintf+0x60>
        printint(fd, va_arg(ap, int), 16, 0);
 890:	008b0913          	addi	s2,s6,8
 894:	4681                	li	a3,0
 896:	4641                	li	a2,16
 898:	000b2583          	lw	a1,0(s6)
 89c:	8556                	mv	a0,s5
 89e:	00000097          	auipc	ra,0x0
 8a2:	e56080e7          	jalr	-426(ra) # 6f4 <printint>
 8a6:	8b4a                	mv	s6,s2
      state = 0;
 8a8:	4981                	li	s3,0
 8aa:	bf91                	j	7fe <vprintf+0x60>
        printptr(fd, va_arg(ap, uint64));
 8ac:	008b0793          	addi	a5,s6,8
 8b0:	f8f43423          	sd	a5,-120(s0)
 8b4:	000b3983          	ld	s3,0(s6)
  putc(fd, '0');
 8b8:	03000593          	li	a1,48
 8bc:	8556                	mv	a0,s5
 8be:	00000097          	auipc	ra,0x0
 8c2:	e14080e7          	jalr	-492(ra) # 6d2 <putc>
  putc(fd, 'x');
 8c6:	85ea                	mv	a1,s10
 8c8:	8556                	mv	a0,s5
 8ca:	00000097          	auipc	ra,0x0
 8ce:	e08080e7          	jalr	-504(ra) # 6d2 <putc>
 8d2:	4941                	li	s2,16
    putc(fd, digits[x >> (sizeof(uint64) * 8 - 4)]);
 8d4:	03c9d793          	srli	a5,s3,0x3c
 8d8:	97de                	add	a5,a5,s7
 8da:	0007c583          	lbu	a1,0(a5)
 8de:	8556                	mv	a0,s5
 8e0:	00000097          	auipc	ra,0x0
 8e4:	df2080e7          	jalr	-526(ra) # 6d2 <putc>
  for (i = 0; i < (sizeof(uint64) * 2); i++, x <<= 4)
 8e8:	0992                	slli	s3,s3,0x4
 8ea:	397d                	addiw	s2,s2,-1
 8ec:	fe0914e3          	bnez	s2,8d4 <vprintf+0x136>
        printptr(fd, va_arg(ap, uint64));
 8f0:	f8843b03          	ld	s6,-120(s0)
      state = 0;
 8f4:	4981                	li	s3,0
 8f6:	b721                	j	7fe <vprintf+0x60>
        s = va_arg(ap, char*);
 8f8:	008b0993          	addi	s3,s6,8
 8fc:	000b3903          	ld	s2,0(s6)
        if(s == 0)
 900:	02090163          	beqz	s2,922 <vprintf+0x184>
        while(*s != 0){
 904:	00094583          	lbu	a1,0(s2)
 908:	c9a1                	beqz	a1,958 <vprintf+0x1ba>
          putc(fd, *s);
 90a:	8556                	mv	a0,s5
 90c:	00000097          	auipc	ra,0x0
 910:	dc6080e7          	jalr	-570(ra) # 6d2 <putc>
          s++;
 914:	0905                	addi	s2,s2,1
        while(*s != 0){
 916:	00094583          	lbu	a1,0(s2)
 91a:	f9e5                	bnez	a1,90a <vprintf+0x16c>
        s = va_arg(ap, char*);
 91c:	8b4e                	mv	s6,s3
      state = 0;
 91e:	4981                	li	s3,0
 920:	bdf9                	j	7fe <vprintf+0x60>
          s = "(null)";
 922:	00000917          	auipc	s2,0x0
 926:	6e690913          	addi	s2,s2,1766 # 1008 <thread_kill+0x88>
        while(*s != 0){
 92a:	02800593          	li	a1,40
 92e:	bff1                	j	90a <vprintf+0x16c>
        putc(fd, va_arg(ap, uint));
 930:	008b0913          	addi	s2,s6,8
 934:	000b4583          	lbu	a1,0(s6)
 938:	8556                	mv	a0,s5
 93a:	00000097          	auipc	ra,0x0
 93e:	d98080e7          	jalr	-616(ra) # 6d2 <putc>
 942:	8b4a                	mv	s6,s2
      state = 0;
 944:	4981                	li	s3,0
 946:	bd65                	j	7fe <vprintf+0x60>
        putc(fd, c);
 948:	85d2                	mv	a1,s4
 94a:	8556                	mv	a0,s5
 94c:	00000097          	auipc	ra,0x0
 950:	d86080e7          	jalr	-634(ra) # 6d2 <putc>
      state = 0;
 954:	4981                	li	s3,0
 956:	b565                	j	7fe <vprintf+0x60>
        s = va_arg(ap, char*);
 958:	8b4e                	mv	s6,s3
      state = 0;
 95a:	4981                	li	s3,0
 95c:	b54d                	j	7fe <vprintf+0x60>
    }
  }
}
 95e:	70e6                	ld	ra,120(sp)
 960:	7446                	ld	s0,112(sp)
 962:	74a6                	ld	s1,104(sp)
 964:	7906                	ld	s2,96(sp)
 966:	69e6                	ld	s3,88(sp)
 968:	6a46                	ld	s4,80(sp)
 96a:	6aa6                	ld	s5,72(sp)
 96c:	6b06                	ld	s6,64(sp)
 96e:	7be2                	ld	s7,56(sp)
 970:	7c42                	ld	s8,48(sp)
 972:	7ca2                	ld	s9,40(sp)
 974:	7d02                	ld	s10,32(sp)
 976:	6de2                	ld	s11,24(sp)
 978:	6109                	addi	sp,sp,128
 97a:	8082                	ret

000000000000097c <fprintf>:

void
fprintf(int fd, const char *fmt, ...)
{
 97c:	715d                	addi	sp,sp,-80
 97e:	ec06                	sd	ra,24(sp)
 980:	e822                	sd	s0,16(sp)
 982:	1000                	addi	s0,sp,32
 984:	e010                	sd	a2,0(s0)
 986:	e414                	sd	a3,8(s0)
 988:	e818                	sd	a4,16(s0)
 98a:	ec1c                	sd	a5,24(s0)
 98c:	03043023          	sd	a6,32(s0)
 990:	03143423          	sd	a7,40(s0)
  va_list ap;

  va_start(ap, fmt);
 994:	fe843423          	sd	s0,-24(s0)
  vprintf(fd, fmt, ap);
 998:	8622                	mv	a2,s0
 99a:	00000097          	auipc	ra,0x0
 99e:	e04080e7          	jalr	-508(ra) # 79e <vprintf>
}
 9a2:	60e2                	ld	ra,24(sp)
 9a4:	6442                	ld	s0,16(sp)
 9a6:	6161                	addi	sp,sp,80
 9a8:	8082                	ret

00000000000009aa <printf>:

void
printf(const char *fmt, ...)
{
 9aa:	711d                	addi	sp,sp,-96
 9ac:	ec06                	sd	ra,24(sp)
 9ae:	e822                	sd	s0,16(sp)
 9b0:	1000                	addi	s0,sp,32
 9b2:	e40c                	sd	a1,8(s0)
 9b4:	e810                	sd	a2,16(s0)
 9b6:	ec14                	sd	a3,24(s0)
 9b8:	f018                	sd	a4,32(s0)
 9ba:	f41c                	sd	a5,40(s0)
 9bc:	03043823          	sd	a6,48(s0)
 9c0:	03143c23          	sd	a7,56(s0)
  va_list ap;

  va_start(ap, fmt);
 9c4:	00840613          	addi	a2,s0,8
 9c8:	fec43423          	sd	a2,-24(s0)
  vprintf(1, fmt, ap);
 9cc:	85aa                	mv	a1,a0
 9ce:	4505                	li	a0,1
 9d0:	00000097          	auipc	ra,0x0
 9d4:	dce080e7          	jalr	-562(ra) # 79e <vprintf>
}
 9d8:	60e2                	ld	ra,24(sp)
 9da:	6442                	ld	s0,16(sp)
 9dc:	6125                	addi	sp,sp,96
 9de:	8082                	ret

00000000000009e0 <free>:
static Header base;
static Header *freep;

void
free(void *ap)
{
 9e0:	1141                	addi	sp,sp,-16
 9e2:	e422                	sd	s0,8(sp)
 9e4:	0800                	addi	s0,sp,16
  Header *bp, *p;

  bp = (Header*)ap - 1;
 9e6:	ff050693          	addi	a3,a0,-16
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
 9ea:	00000797          	auipc	a5,0x0
 9ee:	63e7b783          	ld	a5,1598(a5) # 1028 <freep>
 9f2:	a805                	j	a22 <free+0x42>
    if(p >= p->s.ptr && (bp > p || bp < p->s.ptr))
      break;
  if(bp + bp->s.size == p->s.ptr){
    bp->s.size += p->s.ptr->s.size;
 9f4:	4618                	lw	a4,8(a2)
 9f6:	9db9                	addw	a1,a1,a4
 9f8:	feb52c23          	sw	a1,-8(a0)
    bp->s.ptr = p->s.ptr->s.ptr;
 9fc:	6398                	ld	a4,0(a5)
 9fe:	6318                	ld	a4,0(a4)
 a00:	fee53823          	sd	a4,-16(a0)
 a04:	a091                	j	a48 <free+0x68>
  } else
    bp->s.ptr = p->s.ptr;
  if(p + p->s.size == bp){
    p->s.size += bp->s.size;
 a06:	ff852703          	lw	a4,-8(a0)
 a0a:	9e39                	addw	a2,a2,a4
 a0c:	c790                	sw	a2,8(a5)
    p->s.ptr = bp->s.ptr;
 a0e:	ff053703          	ld	a4,-16(a0)
 a12:	e398                	sd	a4,0(a5)
 a14:	a099                	j	a5a <free+0x7a>
    if(p >= p->s.ptr && (bp > p || bp < p->s.ptr))
 a16:	6398                	ld	a4,0(a5)
 a18:	00e7e463          	bltu	a5,a4,a20 <free+0x40>
 a1c:	00e6ea63          	bltu	a3,a4,a30 <free+0x50>
{
 a20:	87ba                	mv	a5,a4
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
 a22:	fed7fae3          	bgeu	a5,a3,a16 <free+0x36>
 a26:	6398                	ld	a4,0(a5)
 a28:	00e6e463          	bltu	a3,a4,a30 <free+0x50>
    if(p >= p->s.ptr && (bp > p || bp < p->s.ptr))
 a2c:	fee7eae3          	bltu	a5,a4,a20 <free+0x40>
  if(bp + bp->s.size == p->s.ptr){
 a30:	ff852583          	lw	a1,-8(a0)
 a34:	6390                	ld	a2,0(a5)
 a36:	02059713          	slli	a4,a1,0x20
 a3a:	9301                	srli	a4,a4,0x20
 a3c:	0712                	slli	a4,a4,0x4
 a3e:	9736                	add	a4,a4,a3
 a40:	fae60ae3          	beq	a2,a4,9f4 <free+0x14>
    bp->s.ptr = p->s.ptr;
 a44:	fec53823          	sd	a2,-16(a0)
  if(p + p->s.size == bp){
 a48:	4790                	lw	a2,8(a5)
 a4a:	02061713          	slli	a4,a2,0x20
 a4e:	9301                	srli	a4,a4,0x20
 a50:	0712                	slli	a4,a4,0x4
 a52:	973e                	add	a4,a4,a5
 a54:	fae689e3          	beq	a3,a4,a06 <free+0x26>
  } else
    p->s.ptr = bp;
 a58:	e394                	sd	a3,0(a5)
  freep = p;
 a5a:	00000717          	auipc	a4,0x0
 a5e:	5cf73723          	sd	a5,1486(a4) # 1028 <freep>
}
 a62:	6422                	ld	s0,8(sp)
 a64:	0141                	addi	sp,sp,16
 a66:	8082                	ret

0000000000000a68 <malloc>:
  return freep;
}

void*
malloc(uint nbytes)
{
 a68:	7139                	addi	sp,sp,-64
 a6a:	fc06                	sd	ra,56(sp)
 a6c:	f822                	sd	s0,48(sp)
 a6e:	f426                	sd	s1,40(sp)
 a70:	f04a                	sd	s2,32(sp)
 a72:	ec4e                	sd	s3,24(sp)
 a74:	e852                	sd	s4,16(sp)
 a76:	e456                	sd	s5,8(sp)
 a78:	e05a                	sd	s6,0(sp)
 a7a:	0080                	addi	s0,sp,64
  Header *p, *prevp;
  uint nunits;

  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 a7c:	02051493          	slli	s1,a0,0x20
 a80:	9081                	srli	s1,s1,0x20
 a82:	04bd                	addi	s1,s1,15
 a84:	8091                	srli	s1,s1,0x4
 a86:	0014899b          	addiw	s3,s1,1
 a8a:	0485                	addi	s1,s1,1
  if((prevp = freep) == 0){
 a8c:	00000517          	auipc	a0,0x0
 a90:	59c53503          	ld	a0,1436(a0) # 1028 <freep>
 a94:	c515                	beqz	a0,ac0 <malloc+0x58>
    base.s.ptr = freep = prevp = &base;
    base.s.size = 0;
  }
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 a96:	611c                	ld	a5,0(a0)
    if(p->s.size >= nunits){
 a98:	4798                	lw	a4,8(a5)
 a9a:	02977f63          	bgeu	a4,s1,ad8 <malloc+0x70>
 a9e:	8a4e                	mv	s4,s3
 aa0:	0009871b          	sext.w	a4,s3
 aa4:	6685                	lui	a3,0x1
 aa6:	00d77363          	bgeu	a4,a3,aac <malloc+0x44>
 aaa:	6a05                	lui	s4,0x1
 aac:	000a0b1b          	sext.w	s6,s4
  p = sbrk(nu * sizeof(Header));
 ab0:	004a1a1b          	slliw	s4,s4,0x4
        p->s.size = nunits;
      }
      freep = prevp;
      return (void*)(p + 1);
    }
    if(p == freep)
 ab4:	00000917          	auipc	s2,0x0
 ab8:	57490913          	addi	s2,s2,1396 # 1028 <freep>
  if(p == (char*)-1)
 abc:	5afd                	li	s5,-1
 abe:	a88d                	j	b30 <malloc+0xc8>
    base.s.ptr = freep = prevp = &base;
 ac0:	00000797          	auipc	a5,0x0
 ac4:	57878793          	addi	a5,a5,1400 # 1038 <base>
 ac8:	00000717          	auipc	a4,0x0
 acc:	56f73023          	sd	a5,1376(a4) # 1028 <freep>
 ad0:	e39c                	sd	a5,0(a5)
    base.s.size = 0;
 ad2:	0007a423          	sw	zero,8(a5)
    if(p->s.size >= nunits){
 ad6:	b7e1                	j	a9e <malloc+0x36>
      if(p->s.size == nunits)
 ad8:	02e48b63          	beq	s1,a4,b0e <malloc+0xa6>
        p->s.size -= nunits;
 adc:	4137073b          	subw	a4,a4,s3
 ae0:	c798                	sw	a4,8(a5)
        p += p->s.size;
 ae2:	1702                	slli	a4,a4,0x20
 ae4:	9301                	srli	a4,a4,0x20
 ae6:	0712                	slli	a4,a4,0x4
 ae8:	97ba                	add	a5,a5,a4
        p->s.size = nunits;
 aea:	0137a423          	sw	s3,8(a5)
      freep = prevp;
 aee:	00000717          	auipc	a4,0x0
 af2:	52a73d23          	sd	a0,1338(a4) # 1028 <freep>
      return (void*)(p + 1);
 af6:	01078513          	addi	a0,a5,16
      if((p = morecore(nunits)) == 0)
        return 0;
  }
}
 afa:	70e2                	ld	ra,56(sp)
 afc:	7442                	ld	s0,48(sp)
 afe:	74a2                	ld	s1,40(sp)
 b00:	7902                	ld	s2,32(sp)
 b02:	69e2                	ld	s3,24(sp)
 b04:	6a42                	ld	s4,16(sp)
 b06:	6aa2                	ld	s5,8(sp)
 b08:	6b02                	ld	s6,0(sp)
 b0a:	6121                	addi	sp,sp,64
 b0c:	8082                	ret
        prevp->s.ptr = p->s.ptr;
 b0e:	6398                	ld	a4,0(a5)
 b10:	e118                	sd	a4,0(a0)
 b12:	bff1                	j	aee <malloc+0x86>
  hp->s.size = nu;
 b14:	01652423          	sw	s6,8(a0)
  free((void*)(hp + 1));
 b18:	0541                	addi	a0,a0,16
 b1a:	00000097          	auipc	ra,0x0
 b1e:	ec6080e7          	jalr	-314(ra) # 9e0 <free>
  return freep;
 b22:	00093503          	ld	a0,0(s2)
      if((p = morecore(nunits)) == 0)
 b26:	d971                	beqz	a0,afa <malloc+0x92>
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 b28:	611c                	ld	a5,0(a0)
    if(p->s.size >= nunits){
 b2a:	4798                	lw	a4,8(a5)
 b2c:	fa9776e3          	bgeu	a4,s1,ad8 <malloc+0x70>
    if(p == freep)
 b30:	00093703          	ld	a4,0(s2)
 b34:	853e                	mv	a0,a5
 b36:	fef719e3          	bne	a4,a5,b28 <malloc+0xc0>
  p = sbrk(nu * sizeof(Header));
 b3a:	8552                	mv	a0,s4
 b3c:	00000097          	auipc	ra,0x0
 b40:	b7e080e7          	jalr	-1154(ra) # 6ba <sbrk>
  if(p == (char*)-1)
 b44:	fd5518e3          	bne	a0,s5,b14 <malloc+0xac>
        return 0;
 b48:	4501                	li	a0,0
 b4a:	bf45                	j	afa <malloc+0x92>

0000000000000b4c <setjmp>:
 b4c:	e100                	sd	s0,0(a0)
 b4e:	e504                	sd	s1,8(a0)
 b50:	01253823          	sd	s2,16(a0)
 b54:	01353c23          	sd	s3,24(a0)
 b58:	03453023          	sd	s4,32(a0)
 b5c:	03553423          	sd	s5,40(a0)
 b60:	03653823          	sd	s6,48(a0)
 b64:	03753c23          	sd	s7,56(a0)
 b68:	05853023          	sd	s8,64(a0)
 b6c:	05953423          	sd	s9,72(a0)
 b70:	05a53823          	sd	s10,80(a0)
 b74:	05b53c23          	sd	s11,88(a0)
 b78:	06153023          	sd	ra,96(a0)
 b7c:	06253423          	sd	sp,104(a0)
 b80:	4501                	li	a0,0
 b82:	8082                	ret

0000000000000b84 <longjmp>:
 b84:	6100                	ld	s0,0(a0)
 b86:	6504                	ld	s1,8(a0)
 b88:	01053903          	ld	s2,16(a0)
 b8c:	01853983          	ld	s3,24(a0)
 b90:	02053a03          	ld	s4,32(a0)
 b94:	02853a83          	ld	s5,40(a0)
 b98:	03053b03          	ld	s6,48(a0)
 b9c:	03853b83          	ld	s7,56(a0)
 ba0:	04053c03          	ld	s8,64(a0)
 ba4:	04853c83          	ld	s9,72(a0)
 ba8:	05053d03          	ld	s10,80(a0)
 bac:	05853d83          	ld	s11,88(a0)
 bb0:	06053083          	ld	ra,96(a0)
 bb4:	06853103          	ld	sp,104(a0)
 bb8:	c199                	beqz	a1,bbe <longjmp_1>
 bba:	852e                	mv	a0,a1
 bbc:	8082                	ret

0000000000000bbe <longjmp_1>:
 bbe:	4505                	li	a0,1
 bc0:	8082                	ret

0000000000000bc2 <thread_create>:
static struct thread* current_thread = NULL;
static int id = 1;
static jmp_buf env_st;
static jmp_buf env_tmp;

struct thread *thread_create(void (*f)(void *), void *arg){
 bc2:	7179                	addi	sp,sp,-48
 bc4:	f406                	sd	ra,40(sp)
 bc6:	f022                	sd	s0,32(sp)
 bc8:	ec26                	sd	s1,24(sp)
 bca:	e84a                	sd	s2,16(sp)
 bcc:	e44e                	sd	s3,8(sp)
 bce:	1800                	addi	s0,sp,48
 bd0:	89aa                	mv	s3,a0
 bd2:	892e                	mv	s2,a1
    struct thread *t = (struct thread*) malloc(sizeof(struct thread));
 bd4:	13800513          	li	a0,312
 bd8:	00000097          	auipc	ra,0x0
 bdc:	e90080e7          	jalr	-368(ra) # a68 <malloc>
 be0:	84aa                	mv	s1,a0
    unsigned long new_stack_p;
    unsigned long new_stack;
    new_stack = (unsigned long) malloc(sizeof(unsigned long)*0x100);
 be2:	6505                	lui	a0,0x1
 be4:	80050513          	addi	a0,a0,-2048 # 800 <vprintf+0x62>
 be8:	00000097          	auipc	ra,0x0
 bec:	e80080e7          	jalr	-384(ra) # a68 <malloc>
    new_stack_p = new_stack +0x100*8-0x2*8;
    t->fp = f;
 bf0:	0134b023          	sd	s3,0(s1)
    t->arg = arg;
 bf4:	0124b423          	sd	s2,8(s1)
    t->ID  = id;
 bf8:	00000717          	auipc	a4,0x0
 bfc:	42c70713          	addi	a4,a4,1068 # 1024 <id>
 c00:	431c                	lw	a5,0(a4)
 c02:	08f4aa23          	sw	a5,148(s1)
    t->buf_set = 0;
 c06:	0804a823          	sw	zero,144(s1)
    t->stack = (void*) new_stack;
 c0a:	e888                	sd	a0,16(s1)
    new_stack_p = new_stack +0x100*8-0x2*8;
 c0c:	7f050513          	addi	a0,a0,2032
    t->stack_p = (void*) new_stack_p;
 c10:	ec88                	sd	a0,24(s1)
    id++;
 c12:	2785                	addiw	a5,a5,1
 c14:	c31c                	sw	a5,0(a4)

    // part 2
    t->sig_handler[0] = NULL_FUNC;
 c16:	57fd                	li	a5,-1
 c18:	f4dc                	sd	a5,168(s1)
    t->sig_handler[1] = NULL_FUNC;
 c1a:	f8dc                	sd	a5,176(s1)
    t->signo = -1;
 c1c:	0af4ac23          	sw	a5,184(s1)
    t->handler_buf_set = 0;
 c20:	1204a823          	sw	zero,304(s1)
    return t;
}
 c24:	8526                	mv	a0,s1
 c26:	70a2                	ld	ra,40(sp)
 c28:	7402                	ld	s0,32(sp)
 c2a:	64e2                	ld	s1,24(sp)
 c2c:	6942                	ld	s2,16(sp)
 c2e:	69a2                	ld	s3,8(sp)
 c30:	6145                	addi	sp,sp,48
 c32:	8082                	ret

0000000000000c34 <thread_add_runqueue>:
void thread_add_runqueue(struct thread *t){
 c34:	1141                	addi	sp,sp,-16
 c36:	e422                	sd	s0,8(sp)
 c38:	0800                	addi	s0,sp,16
    if(current_thread == NULL){
 c3a:	00000797          	auipc	a5,0x0
 c3e:	3f67b783          	ld	a5,1014(a5) # 1030 <current_thread>
 c42:	c785                	beqz	a5,c6a <thread_add_runqueue+0x36>
        current_thread = t;
    }
    else{
        // TODO
        // add the thread to the non-empty runqueue
        current_thread->previous->next = t;
 c44:	6fd8                	ld	a4,152(a5)
 c46:	f348                	sd	a0,160(a4)
        t->previous = current_thread->previous;
 c48:	6fd8                	ld	a4,152(a5)
 c4a:	ed58                	sd	a4,152(a0)
        
        t->next = current_thread;
 c4c:	f15c                	sd	a5,160(a0)
        current_thread->previous = t;
 c4e:	efc8                	sd	a0,152(a5)

        // inherit signal handler from parent thread
        if(current_thread->sig_handler[0] != NULL_FUNC){
 c50:	77d8                	ld	a4,168(a5)
 c52:	56fd                	li	a3,-1
 c54:	00d70363          	beq	a4,a3,c5a <thread_add_runqueue+0x26>
            t->sig_handler[0] = current_thread->sig_handler[0];
 c58:	f558                	sd	a4,168(a0)
        }
        if(current_thread->sig_handler[1] != NULL_FUNC){
 c5a:	7bdc                	ld	a5,176(a5)
 c5c:	577d                	li	a4,-1
 c5e:	00e78363          	beq	a5,a4,c64 <thread_add_runqueue+0x30>
            t->sig_handler[1] = current_thread->sig_handler[1];
 c62:	f95c                	sd	a5,176(a0)
        }
    }
}
 c64:	6422                	ld	s0,8(sp)
 c66:	0141                	addi	sp,sp,16
 c68:	8082                	ret
        t->previous = t;
 c6a:	ed48                	sd	a0,152(a0)
        t->next = t;
 c6c:	f148                	sd	a0,160(a0)
        current_thread = t;
 c6e:	00000797          	auipc	a5,0x0
 c72:	3ca7b123          	sd	a0,962(a5) # 1030 <current_thread>
 c76:	b7fd                	j	c64 <thread_add_runqueue+0x30>

0000000000000c78 <schedule>:
            // current thread was executed before
            longjmp(current_thread->env, 1);
        }
    }
}
void schedule(void){
 c78:	1141                	addi	sp,sp,-16
 c7a:	e422                	sd	s0,8(sp)
 c7c:	0800                	addi	s0,sp,16
    // TODO
    current_thread = current_thread->next;
 c7e:	00000797          	auipc	a5,0x0
 c82:	3b278793          	addi	a5,a5,946 # 1030 <current_thread>
 c86:	6398                	ld	a4,0(a5)
 c88:	7358                	ld	a4,160(a4)
 c8a:	e398                	sd	a4,0(a5)
}
 c8c:	6422                	ld	s0,8(sp)
 c8e:	0141                	addi	sp,sp,16
 c90:	8082                	ret

0000000000000c92 <thread_exit>:
void thread_exit(void){
 c92:	1101                	addi	sp,sp,-32
 c94:	ec06                	sd	ra,24(sp)
 c96:	e822                	sd	s0,16(sp)
 c98:	e426                	sd	s1,8(sp)
 c9a:	1000                	addi	s0,sp,32
    if(current_thread->next != current_thread){
 c9c:	00000497          	auipc	s1,0x0
 ca0:	3944b483          	ld	s1,916(s1) # 1030 <current_thread>
 ca4:	70dc                	ld	a5,160(s1)
 ca6:	02f48e63          	beq	s1,a5,ce2 <thread_exit+0x50>
        // TODO
        // at least one thread remaining to be executed
        // remove calling thread from runqueue and update current thread
        current_thread->previous->next = current_thread->next;
 caa:	6cd8                	ld	a4,152(s1)
 cac:	f35c                	sd	a5,160(a4)
        current_thread->next->previous = current_thread->previous;
 cae:	6cd8                	ld	a4,152(s1)
 cb0:	efd8                	sd	a4,152(a5)
        
        struct thread *curr = current_thread;
        current_thread = current_thread->next;
 cb2:	70dc                	ld	a5,160(s1)
 cb4:	00000717          	auipc	a4,0x0
 cb8:	36f73e23          	sd	a5,892(a4) # 1030 <current_thread>
        
        free(curr->stack);
 cbc:	6888                	ld	a0,16(s1)
 cbe:	00000097          	auipc	ra,0x0
 cc2:	d22080e7          	jalr	-734(ra) # 9e0 <free>
        free(curr);
 cc6:	8526                	mv	a0,s1
 cc8:	00000097          	auipc	ra,0x0
 ccc:	d18080e7          	jalr	-744(ra) # 9e0 <free>

        dispatch();
 cd0:	00000097          	auipc	ra,0x0
 cd4:	046080e7          	jalr	70(ra) # d16 <dispatch>
        free(current_thread->stack);
        free(current_thread);
        current_thread = NULL;
        longjmp(env_st, 1);
    }
}
 cd8:	60e2                	ld	ra,24(sp)
 cda:	6442                	ld	s0,16(sp)
 cdc:	64a2                	ld	s1,8(sp)
 cde:	6105                	addi	sp,sp,32
 ce0:	8082                	ret
        free(current_thread->stack);
 ce2:	6888                	ld	a0,16(s1)
 ce4:	00000097          	auipc	ra,0x0
 ce8:	cfc080e7          	jalr	-772(ra) # 9e0 <free>
        free(current_thread);
 cec:	00000497          	auipc	s1,0x0
 cf0:	34448493          	addi	s1,s1,836 # 1030 <current_thread>
 cf4:	6088                	ld	a0,0(s1)
 cf6:	00000097          	auipc	ra,0x0
 cfa:	cea080e7          	jalr	-790(ra) # 9e0 <free>
        current_thread = NULL;
 cfe:	0004b023          	sd	zero,0(s1)
        longjmp(env_st, 1);
 d02:	4585                	li	a1,1
 d04:	00000517          	auipc	a0,0x0
 d08:	34450513          	addi	a0,a0,836 # 1048 <env_st>
 d0c:	00000097          	auipc	ra,0x0
 d10:	e78080e7          	jalr	-392(ra) # b84 <longjmp>
}
 d14:	b7d1                	j	cd8 <thread_exit+0x46>

0000000000000d16 <dispatch>:
void dispatch(void){
 d16:	1101                	addi	sp,sp,-32
 d18:	ec06                	sd	ra,24(sp)
 d1a:	e822                	sd	s0,16(sp)
 d1c:	e426                	sd	s1,8(sp)
 d1e:	1000                	addi	s0,sp,32
    if(current_thread->signo == 0 || current_thread->signo == 1){
 d20:	00000517          	auipc	a0,0x0
 d24:	31053503          	ld	a0,784(a0) # 1030 <current_thread>
 d28:	0b852783          	lw	a5,184(a0)
 d2c:	0007869b          	sext.w	a3,a5
 d30:	4705                	li	a4,1
 d32:	04d77063          	bgeu	a4,a3,d72 <dispatch+0x5c>
        if(current_thread->buf_set == 0){
 d36:	09052783          	lw	a5,144(a0)
 d3a:	16079163          	bnez	a5,e9c <dispatch+0x186>
            sj_ret = setjmp(env_tmp);
 d3e:	00000517          	auipc	a0,0x0
 d42:	37a50513          	addi	a0,a0,890 # 10b8 <env_tmp>
 d46:	00000097          	auipc	ra,0x0
 d4a:	e06080e7          	jalr	-506(ra) # b4c <setjmp>
            if(sj_ret == 0){
 d4e:	12050463          	beqz	a0,e76 <dispatch+0x160>
                current_thread->fp(current_thread->arg);
 d52:	00000797          	auipc	a5,0x0
 d56:	2de7b783          	ld	a5,734(a5) # 1030 <current_thread>
 d5a:	6398                	ld	a4,0(a5)
 d5c:	6788                	ld	a0,8(a5)
 d5e:	9702                	jalr	a4
                thread_exit();
 d60:	00000097          	auipc	ra,0x0
 d64:	f32080e7          	jalr	-206(ra) # c92 <thread_exit>
}
 d68:	60e2                	ld	ra,24(sp)
 d6a:	6442                	ld	s0,16(sp)
 d6c:	64a2                	ld	s1,8(sp)
 d6e:	6105                	addi	sp,sp,32
 d70:	8082                	ret
        if(current_thread->sig_handler[current_thread->signo] == NULL_FUNC){
 d72:	07d1                	addi	a5,a5,20
 d74:	078e                	slli	a5,a5,0x3
 d76:	97aa                	add	a5,a5,a0
 d78:	6798                	ld	a4,8(a5)
 d7a:	57fd                	li	a5,-1
 d7c:	04f70863          	beq	a4,a5,dcc <dispatch+0xb6>
        if(current_thread->handler_buf_set == 0){
 d80:	00000517          	auipc	a0,0x0
 d84:	2b053503          	ld	a0,688(a0) # 1030 <current_thread>
 d88:	13052783          	lw	a5,304(a0)
 d8c:	efe9                	bnez	a5,e66 <dispatch+0x150>
            sj_ret = setjmp(env_tmp);
 d8e:	00000517          	auipc	a0,0x0
 d92:	32a50513          	addi	a0,a0,810 # 10b8 <env_tmp>
 d96:	00000097          	auipc	ra,0x0
 d9a:	db6080e7          	jalr	-586(ra) # b4c <setjmp>
            if(sj_ret == 0){
 d9e:	ed15                	bnez	a0,dda <dispatch+0xc4>
                if(current_thread->buf_set == 0){
 da0:	00000797          	auipc	a5,0x0
 da4:	2907b783          	ld	a5,656(a5) # 1030 <current_thread>
 da8:	0907a703          	lw	a4,144(a5)
 dac:	e70d                	bnez	a4,dd6 <dispatch+0xc0>
                    env_tmp->sp = (unsigned long)current_thread->stack_p;
 dae:	6f9c                	ld	a5,24(a5)
 db0:	00000717          	auipc	a4,0x0
 db4:	36f73823          	sd	a5,880(a4) # 1120 <env_tmp+0x68>
                longjmp(env_tmp, 1);
 db8:	4585                	li	a1,1
 dba:	00000517          	auipc	a0,0x0
 dbe:	2fe50513          	addi	a0,a0,766 # 10b8 <env_tmp>
 dc2:	00000097          	auipc	ra,0x0
 dc6:	dc2080e7          	jalr	-574(ra) # b84 <longjmp>
 dca:	bf79                	j	d68 <dispatch+0x52>
            thread_exit();
 dcc:	00000097          	auipc	ra,0x0
 dd0:	ec6080e7          	jalr	-314(ra) # c92 <thread_exit>
 dd4:	b775                	j	d80 <dispatch+0x6a>
                    env_tmp->sp = current_thread->env->sp;
 dd6:	67dc                	ld	a5,136(a5)
 dd8:	bfe1                	j	db0 <dispatch+0x9a>
                current_thread->sig_handler[current_thread->signo](current_thread->signo);
 dda:	00000497          	auipc	s1,0x0
 dde:	25648493          	addi	s1,s1,598 # 1030 <current_thread>
 de2:	609c                	ld	a5,0(s1)
 de4:	0b87a503          	lw	a0,184(a5)
 de8:	01450713          	addi	a4,a0,20
 dec:	070e                	slli	a4,a4,0x3
 dee:	97ba                	add	a5,a5,a4
 df0:	679c                	ld	a5,8(a5)
 df2:	9782                	jalr	a5
                current_thread->signo = -1;
 df4:	6088                	ld	a0,0(s1)
 df6:	57fd                	li	a5,-1
 df8:	0af52c23          	sw	a5,184(a0)
                if(current_thread->buf_set == 1){
 dfc:	09052703          	lw	a4,144(a0)
 e00:	4785                	li	a5,1
 e02:	02f70763          	beq	a4,a5,e30 <dispatch+0x11a>
                    sj_ret = setjmp(env_tmp);
 e06:	00000517          	auipc	a0,0x0
 e0a:	2b250513          	addi	a0,a0,690 # 10b8 <env_tmp>
 e0e:	00000097          	auipc	ra,0x0
 e12:	d3e080e7          	jalr	-706(ra) # b4c <setjmp>
                    if(sj_ret == 0){
 e16:	c50d                	beqz	a0,e40 <dispatch+0x12a>
                        current_thread->fp(current_thread->arg);
 e18:	00000797          	auipc	a5,0x0
 e1c:	2187b783          	ld	a5,536(a5) # 1030 <current_thread>
 e20:	6398                	ld	a4,0(a5)
 e22:	6788                	ld	a0,8(a5)
 e24:	9702                	jalr	a4
                        thread_exit();
 e26:	00000097          	auipc	ra,0x0
 e2a:	e6c080e7          	jalr	-404(ra) # c92 <thread_exit>
 e2e:	bf2d                	j	d68 <dispatch+0x52>
                    longjmp(current_thread->env, 1);
 e30:	4585                	li	a1,1
 e32:	02050513          	addi	a0,a0,32
 e36:	00000097          	auipc	ra,0x0
 e3a:	d4e080e7          	jalr	-690(ra) # b84 <longjmp>
 e3e:	b72d                	j	d68 <dispatch+0x52>
                        env_tmp->sp = (unsigned long)current_thread->stack_p;
 e40:	00000797          	auipc	a5,0x0
 e44:	1f07b783          	ld	a5,496(a5) # 1030 <current_thread>
 e48:	6f9c                	ld	a5,24(a5)
 e4a:	00000717          	auipc	a4,0x0
 e4e:	2cf73b23          	sd	a5,726(a4) # 1120 <env_tmp+0x68>
                        longjmp(env_tmp, 1);
 e52:	4585                	li	a1,1
 e54:	00000517          	auipc	a0,0x0
 e58:	26450513          	addi	a0,a0,612 # 10b8 <env_tmp>
 e5c:	00000097          	auipc	ra,0x0
 e60:	d28080e7          	jalr	-728(ra) # b84 <longjmp>
 e64:	b711                	j	d68 <dispatch+0x52>
            longjmp(current_thread->handler_env, 1);
 e66:	4585                	li	a1,1
 e68:	0c050513          	addi	a0,a0,192
 e6c:	00000097          	auipc	ra,0x0
 e70:	d18080e7          	jalr	-744(ra) # b84 <longjmp>
 e74:	bdd5                	j	d68 <dispatch+0x52>
                env_tmp->sp = (unsigned long)current_thread->stack_p;
 e76:	00000797          	auipc	a5,0x0
 e7a:	1ba7b783          	ld	a5,442(a5) # 1030 <current_thread>
 e7e:	6f9c                	ld	a5,24(a5)
 e80:	00000717          	auipc	a4,0x0
 e84:	2af73023          	sd	a5,672(a4) # 1120 <env_tmp+0x68>
                longjmp(env_tmp, 1);
 e88:	4585                	li	a1,1
 e8a:	00000517          	auipc	a0,0x0
 e8e:	22e50513          	addi	a0,a0,558 # 10b8 <env_tmp>
 e92:	00000097          	auipc	ra,0x0
 e96:	cf2080e7          	jalr	-782(ra) # b84 <longjmp>
 e9a:	b5f9                	j	d68 <dispatch+0x52>
            longjmp(current_thread->env, 1);
 e9c:	4585                	li	a1,1
 e9e:	02050513          	addi	a0,a0,32
 ea2:	00000097          	auipc	ra,0x0
 ea6:	ce2080e7          	jalr	-798(ra) # b84 <longjmp>
}
 eaa:	bd7d                	j	d68 <dispatch+0x52>

0000000000000eac <thread_yield>:
void thread_yield(void){
 eac:	1141                	addi	sp,sp,-16
 eae:	e406                	sd	ra,8(sp)
 eb0:	e022                	sd	s0,0(sp)
 eb2:	0800                	addi	s0,sp,16
    if(current_thread->signo == 0 || current_thread->signo == 1){
 eb4:	00000517          	auipc	a0,0x0
 eb8:	17c53503          	ld	a0,380(a0) # 1030 <current_thread>
 ebc:	0b852703          	lw	a4,184(a0)
 ec0:	4785                	li	a5,1
 ec2:	00e7fd63          	bgeu	a5,a4,edc <thread_yield+0x30>
        sj_ret = setjmp(current_thread->env);
 ec6:	02050513          	addi	a0,a0,32
 eca:	00000097          	auipc	ra,0x0
 ece:	c82080e7          	jalr	-894(ra) # b4c <setjmp>
        if(sj_ret == 0){
 ed2:	cd05                	beqz	a0,f0a <thread_yield+0x5e>
}
 ed4:	60a2                	ld	ra,8(sp)
 ed6:	6402                	ld	s0,0(sp)
 ed8:	0141                	addi	sp,sp,16
 eda:	8082                	ret
        sj_handler_ret = setjmp(current_thread->handler_env);
 edc:	0c050513          	addi	a0,a0,192
 ee0:	00000097          	auipc	ra,0x0
 ee4:	c6c080e7          	jalr	-916(ra) # b4c <setjmp>
        if(sj_handler_ret == 0){
 ee8:	f575                	bnez	a0,ed4 <thread_yield+0x28>
            current_thread->handler_buf_set = 1;
 eea:	00000797          	auipc	a5,0x0
 eee:	1467b783          	ld	a5,326(a5) # 1030 <current_thread>
 ef2:	4705                	li	a4,1
 ef4:	12e7a823          	sw	a4,304(a5)
            schedule();
 ef8:	00000097          	auipc	ra,0x0
 efc:	d80080e7          	jalr	-640(ra) # c78 <schedule>
            dispatch();
 f00:	00000097          	auipc	ra,0x0
 f04:	e16080e7          	jalr	-490(ra) # d16 <dispatch>
 f08:	b7f1                	j	ed4 <thread_yield+0x28>
            current_thread->buf_set = 1;
 f0a:	00000797          	auipc	a5,0x0
 f0e:	1267b783          	ld	a5,294(a5) # 1030 <current_thread>
 f12:	4705                	li	a4,1
 f14:	08e7a823          	sw	a4,144(a5)
            schedule();
 f18:	00000097          	auipc	ra,0x0
 f1c:	d60080e7          	jalr	-672(ra) # c78 <schedule>
            dispatch();
 f20:	00000097          	auipc	ra,0x0
 f24:	df6080e7          	jalr	-522(ra) # d16 <dispatch>
}
 f28:	b775                	j	ed4 <thread_yield+0x28>

0000000000000f2a <thread_start_threading>:
void thread_start_threading(void){
 f2a:	1141                	addi	sp,sp,-16
 f2c:	e406                	sd	ra,8(sp)
 f2e:	e022                	sd	s0,0(sp)
 f30:	0800                	addi	s0,sp,16
    // TODO
    // return only if all threads have exited
    int sj_ret;
    sj_ret = setjmp(env_st);
 f32:	00000517          	auipc	a0,0x0
 f36:	11650513          	addi	a0,a0,278 # 1048 <env_st>
 f3a:	00000097          	auipc	ra,0x0
 f3e:	c12080e7          	jalr	-1006(ra) # b4c <setjmp>
    if(sj_ret == 0){
 f42:	c509                	beqz	a0,f4c <thread_start_threading+0x22>
        dispatch();
    }
}
 f44:	60a2                	ld	ra,8(sp)
 f46:	6402                	ld	s0,0(sp)
 f48:	0141                	addi	sp,sp,16
 f4a:	8082                	ret
        dispatch();
 f4c:	00000097          	auipc	ra,0x0
 f50:	dca080e7          	jalr	-566(ra) # d16 <dispatch>
}
 f54:	bfc5                	j	f44 <thread_start_threading+0x1a>

0000000000000f56 <thread_register_handler>:
// part 2
void thread_register_handler(int signo, void (*handler)(int)){
 f56:	1141                	addi	sp,sp,-16
 f58:	e422                	sd	s0,8(sp)
 f5a:	0800                	addi	s0,sp,16
    // TODO
    // register handler according to signal
    if(signo == 0){
 f5c:	e909                	bnez	a0,f6e <thread_register_handler+0x18>
        current_thread->sig_handler[0] = handler;
 f5e:	00000797          	auipc	a5,0x0
 f62:	0d27b783          	ld	a5,210(a5) # 1030 <current_thread>
 f66:	f7cc                	sd	a1,168(a5)
    }
    else if(signo == 1){
        current_thread->sig_handler[1] = handler;
    }
}
 f68:	6422                	ld	s0,8(sp)
 f6a:	0141                	addi	sp,sp,16
 f6c:	8082                	ret
    else if(signo == 1){
 f6e:	4785                	li	a5,1
 f70:	fef51ce3          	bne	a0,a5,f68 <thread_register_handler+0x12>
        current_thread->sig_handler[1] = handler;
 f74:	00000797          	auipc	a5,0x0
 f78:	0bc7b783          	ld	a5,188(a5) # 1030 <current_thread>
 f7c:	fbcc                	sd	a1,176(a5)
}
 f7e:	b7ed                	j	f68 <thread_register_handler+0x12>

0000000000000f80 <thread_kill>:
void thread_kill(struct thread *t, int signo){
 f80:	1141                	addi	sp,sp,-16
 f82:	e422                	sd	s0,8(sp)
 f84:	0800                	addi	s0,sp,16
    // TODO
    t->signo = signo;
 f86:	0ab52c23          	sw	a1,184(a0)
}
 f8a:	6422                	ld	s0,8(sp)
 f8c:	0141                	addi	sp,sp,16
 f8e:	8082                	ret
